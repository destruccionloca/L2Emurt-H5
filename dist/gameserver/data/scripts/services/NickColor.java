...
public class NickColor extends Functions {
    ...
    public void list() {
        ...
        for (String color : Config.SERVICES_CHANGE_NICK_COLOR_LIST) {
            append.append("<br><a action=\"bypass -h scripts_services.NickColor:change ").append(color).append("\"><font color=\"").append(color.substring(4, 6)).append(color.substring(2, 4)).append(color.substring(0, 2)).append("\">").append(color.substring(4, 6)).append(color.substring(2, 4)).append(color.substring(0, 2)).append("</font></a>");
        }
