...
import l2p.gameserver.model.visual.IItemCond;
...
public class CommunityBoardVisual extends Functions implements ScriptFile, ICommunityBoardHandler {
    ...
    private static final String PAGE_PATH = Config.BBS_HOME_DIR + "pages/visual/";
    ...
    private String parseMainPage(Player player) {
        ...
        if (price.itemId != -1)
            template = ItemHolder.getInstance().getTemplate(price.itemId);
    ...
    private Price getPrice(Player player) {
        ...
        if (consumable.isWeapon())
            return new Price(Config.VISUAL_WEAPON_PRICE_ID, Config.VISUAL_WEAPON_PRICE_COUNT);
        if (VisualUtils.isCostume(consumable.getItemId()))
            return new Price(Config.VISUAL_COSTUME_PRICE_ID, Config.VISUAL_COSTUME_PRICE_COUNT);
        return new Price(Config.VISUAL_ARMOR_PRICE_ID, Config.VISUAL_ARMOR_PRICE_COUNT);
    }
    ...
    private void insert(Player player) {
        ...
        Price price = getPrice(player);
        if (!player.getInventory().destroyItemByItemId(price.itemId, price.itemCount)) {
        ...
        if (player.getInventory().getItemByObjectId(item.getObjectId()) == null) // just in case
            return;
        if (!player.getInventory().destroyItem(consume))
            return;
    ...
    private void remove(Player player) {
        ...
        player.sendPacket(new InventoryUpdate().addModifiedItem(item));
        player.broadcastCharInfo();
        if (Config.VISUAL_RETURN_VISUAL_ITEM)
            player.getInventory().addItem(visualId, 1);
