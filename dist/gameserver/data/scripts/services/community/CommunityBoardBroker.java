...
public class CommunityBoardBroker implements ScriptFile, ICommunityBoardHandler {
    ...
    public void onBypassCommand(Player player, String bypass) {
        ...
        if (bypass.startsWith("_bbsbrokerAllList")) {
            ...
            List<ExchangeItem> items = new ArrayList<>(ExchangeBroker.getExchangeItemList().size() * 10);
            items.addAll(ExchangeBroker.getExchangeItemList().values().stream().collect(Collectors.toList()));
        ...
        if (bypass.startsWith("_bbsbrokerMyList")) {
            ...
            List<ItemInstance> items = new ArrayList<>(player.getInventory().getSize() * 10);
        ...
        if (bypass.startsWith("_bbsbrokerMyItem")) {
            ...
            List<ExchangeItem> items = new ArrayList<>(ExchangeBroker.getItemsForPlayer(ojectId).size() * 10);
            items.addAll(ExchangeBroker.getItemsForPlayer(ojectId).stream().collect(Collectors.toList()));
