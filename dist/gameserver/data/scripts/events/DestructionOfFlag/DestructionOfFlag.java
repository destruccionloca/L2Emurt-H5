...
public class DestructionOfFlag extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    ...
    private static HashMap<Long, List<Effect>> _saveBuffList = new HashMap<>();
    ...
    private static List<Long> players_list1 = new ArrayList<>();
    private static List<Long> players_list2 = new ArrayList<>();
    private static List<Long> players_list3 = new ArrayList<>();
    private static List<Long> players_list4 = new ArrayList<>();
    ...
    private static HashMap<Long, ScheduledFuture<?>> _resurrectionList = new HashMap<>(); //Хранилище задач на Ressuction.
    ...
    public void start(String[] var) {
        ...
        players_list1 = new ArrayList<>();
        players_list2 = new ArrayList<>();
            ...
            if (REMOVE_BUFFS) {
                for (int buff[] : BUFFS_TO_REMOVE) {
                    ...
                    if ((effects = player.getEffectList().getEffectsBySkillId(buff[0])) != null) {
                        if (buff.length == 2) {
                            effects.stream().filter(effect -> effect.getSkill().getLevel() == buff[1]).forEach(effect -> {
    ...
    public static void teleportPlayersToSavedCoords(int command) {
        switch (command) {
            case 1:
                getPlayers(players_list1).forEach(DestructionOfFlag::teleportPlayerToSavedCoords);
                break;
            case 2:
                getPlayers(players_list2).forEach(DestructionOfFlag::teleportPlayerToSavedCoords);
                break;
            case 3:
                getPlayers(players_list3).forEach(DestructionOfFlag::teleportPlayerToSavedCoords);
                break;
            case 4:
                getPlayers(players_list4).forEach(DestructionOfFlag::teleportPlayerToSavedCoords);
                break;
        }
    }
...
//               summon.unSummon();
//       }
        getPlayers(players_list1).stream().filter(player -> player != null).forEach(player -> {
            try {
...
//             summon.unSummon();
//     }
        getPlayers(players_list2).stream().filter(player -> player != null).forEach(player -> {
            try {
                if (player.isCastingNow()) {
                    player.abortCast(true, true);
                }
...
//                summon.unSummon();
//        }
        getPlayers(players_list3).stream().filter(player -> player != null).forEach(player -> {
            try {
                if (player.isCastingNow()) {
                    player.abortCast(true, true);
                }
    ...
    public static void ressurectPlayers() {
        getPlayers(players_list1).forEach(DestructionOfFlag::ressurectPlayer);
        getPlayers(players_list2).forEach(DestructionOfFlag::ressurectPlayer);
        getPlayers(players_list3).forEach(DestructionOfFlag::ressurectPlayer);
        getPlayers(players_list4).forEach(DestructionOfFlag::ressurectPlayer);
    }
            ...
            if (player != null) {
                ...
                for (Effect effect : skill) {
                    if (!_saveBuffList.containsKey(player.getStoredId())) {
                        _saveBuffList.put(player.getStoredId(), new ArrayList<>());
                    }
    ...
    public static class restoreBuffListForPlayer implements Runnable {
        ...
        public void run() {
            ...
            List<Effect> effects = _saveBuffList.get(player.getStoredId());
    ...
    public static void lossTeam(List<Long> team) {
        Player player;
        for (long objId : team) {
            player = GameObjectsStorage.getAsPlayer(objId);
            if (player != null) {
