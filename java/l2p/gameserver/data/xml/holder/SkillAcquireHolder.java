...
public final class SkillAcquireHolder extends AbstractHolder {
    ...
    private static TIntObjectHashMap<List<SkillLearn>> _skillTreeSell = new TIntObjectHashMap<>();
    ...
    public List<SkillLearn> getSkillLearnListByItemId(Player player, int itemId) {
        ...
        l.addAll(learns.stream().filter($i -> $i.getItemId() == itemId).collect(Collectors.toList()));
    ...
    public List<SkillLearn> getAllNormalSkillTreeWithForgottenScrolls() {
        ...
        for (TIntObjectIterator<List<SkillLearn>> i = _normalSkillTree.iterator(); i.hasNext();) {
            i.advance();
            a.addAll(i.value().stream().filter(learn -> learn.getItemId() > 0 && learn.isClicked()).collect(Collectors.toList()));
        }
    ...
    public static TIntObjectHashMap<List<SkillLearn>> getAllNormalSkillForLearns() {
        return _skillTreeSell;
    }
    public static List<SkillLearn> getNormalSkillForLearnsClassId(int classId) {
    ...
    @Override
    public void log() {
        info("load " + sizeTroveMap(_skillTreeSell) + " skillTrade learns for " + _skillTreeSell.size() + " classes.");
