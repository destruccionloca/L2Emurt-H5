        ...
        for (Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();) {
            ...
            for (Iterator<Element> subIterator = element.elementIterator(); subIterator.hasNext();) {
                ...
                if ("level".equalsIgnoreCase(subElement.getName())) {
                    ...
                    subElement.elements().stream().filter(e -> "param".equalsIgnoreCase(e.getName())).forEach(e -> {
                    ...
                    for (Element e : subElement.elements()) {
                        if ("group".equalsIgnoreCase(e.getName())) {
                            ...
                            coords.addAll(e.elements().stream().filter(e2 -> "coords".equalsIgnoreCase(e2.getName())).map(e2 -> Location.parseLoc(e2.attributeValue("loc"))).collect(Collectors.toList()));
