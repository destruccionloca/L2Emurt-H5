...
public final class EffectTransformation extends Effect {
    ...
    public void onExit() {
        ...
        if (_effected.isPlayer()) {
            ...
            if (player.getTransformation() == 258 && player.getPet() != null) // Отзываем пета Divine Beast при выходе из трансформации Tranform Divine Summoner
            {
                if (player.getPet().getNpcId() == 14870) {
                    player.getPet().unSummon();
                }
            }
