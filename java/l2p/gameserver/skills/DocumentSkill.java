...
public final class DocumentSkill extends DocumentBase {
    ...
    public class Skill {
        ...
        public List<l2p.gameserver.model.Skill> skills = new ArrayList<>();
        public List<l2p.gameserver.model.Skill> currentSkills = new ArrayList<>();
    }
    private Skill currentSkill;
    private Set<String> usedTables = new HashSet<>();
    private List<l2p.gameserver.model.Skill> skillsInFile = new LinkedList<>();
    ...
    protected void resetTable() {
        if (!usedTables.isEmpty()) {
            tables.keySet().stream().filter(table -> !usedTables.contains(table)).filter(table -> _log.isDebugEnabled()).forEach(table -> {
    ...
    List<l2p.gameserver.model.Skill> getSkills() {
        return skillsInFile;
    }
