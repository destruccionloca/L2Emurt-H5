...
public class EnterWorld extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.getClan() != null) {
            ...
            if (activeChar.getLevel() < 40 && activeChar.getPledgeType() == -1) {
                ...
                AcademList.getClanList().stream().filter(clanTmp -> clanTmp == clan).forEach(clanTmp -> {
    ...
    private static void notifyClanMembers(Player activeChar) {
		...
		if (Config.OLYMPIAD_PLAYER_HWID && Olympiad._playersHWID.containsKey(activeChar)) {
			Olympiad._playersHWID.remove(activeChar);
		}
