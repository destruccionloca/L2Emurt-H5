...
public class CharacterCreate extends L2GameClientPacket {
    ...
    private void initNewChar(GameClient client, Player newChar) {
        ...
        if (Config.SERVICES_RATE_TYPE != Bonus.NO_BONUS && Config.SERVICES_RATE_CREATE_PA != 0) {
            double bonus = 1.;
            int bonus_type = 1;
            int bonusExpire = (int) (System.currentTimeMillis() / 1000L) + Config.SERVICES_RATE_CREATE_PA * 24 * 60 * 60;
            switch (Config.SERVICES_RATE_TYPE) {
                case Bonus.BONUS_GLOBAL_ON_AUTHSERVER:
                    AuthServerCommunication.getInstance().sendPacket(new BonusRequest(newChar.getAccountName(), bonus_type, bonusExpire));
                    this.getClient().setBonusExpire(bonusExpire);
                    break;
                case Bonus.BONUS_GLOBAL_ON_GAMESERVER:
                    if (AccountBonusDAO.getInstance().ins1(newChar.getAccountName(), bonus_type, bonusExpire)) {
                        this.getClient().setBonusExpire(bonusExpire);
                    }
                    break;
            }
