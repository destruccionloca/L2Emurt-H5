...
public class RequestRecipeShopMakeDo extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (Config.SERVICES_RATE_TYPE != Bonus.NO_BONUS && manufacturer.isOnline() && manufacturer.hasBonus()){
            if (Rnd.chance(PremiumConfig.getPremConfigId(manufacturer.getBonus().getBonusId()).RATE_CRAFT_DOUBLE)) {
                tryCount++;
            }
        ...
        for (int i = 0; i < tryCount; i++) {
            if (Rnd.chance(recipeList.getSuccessRate())) {
                ...
                if (Config.SERVICES_RATE_TYPE != Bonus.NO_BONUS && manufacturer.hasBonus()) {
                    chance = Rnd.chance(PremiumConfig.getPremConfigId(manufacturer.getBonus().getBonusId()).RATE_CRAFT_MW);
                } else {
                    chance = Rnd.chance(Config.CRAFT_MASTERWORK_CHANCE);
                }
