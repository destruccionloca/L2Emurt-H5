...
/**
 * Created by Hack
 * Date: 20.05.2016 16:30
 */
//TODO: перевести на L2HtmlBuilder
    // при отсутствии end() таблицу не завершаем
public class EnchantParser {
    ...
    public void showMainPage(Player player) {
        String page = HtmCache.getInstance().getNotNull("command/enchant.htm", player);
    ...
    public void showItemChoosePage(Player player, int item_type, int sort_type, int page_number) {
        String page = HtmCache.getInstance().getNotNull("command/enchant_item_choose.htm", player);
    ...
    public void showResultPage(Player player, EnchantType type, Map<String, Integer> result) {
        ...
        switch (type) {
            case Scroll:
                page = HtmCache.getInstance().getNotNull("command/enchant_scroll_result.htm", player);
                ...
                break;
            case Attribute:
                page = HtmCache.getInstance().getNotNull("command/enchant_attribute_result.htm", player);
                page = page.replaceFirst("%enchant%", "" + result.get("enchant"));
    ...
    public void showHelpPage(Player player) {
        String page = HtmCache.getInstance().getNotNull("command/enchant_help.htm", player);
        Functions.show(page, player, null);
    }
    ...
    private String parseItemChoosePage(Player player, String page, int itemType, int sort_type, int page_index) {
        int itemsOnPage = 5;
        String content = "";
        StringBuilder template = new StringBuilder(HtmCache.getInstance().getNotNull(itemType == 0 ? "command/enchant_item_obj.htm" : "command/enchant_upgrade_item_obj.htm", player));
        List<ItemInstance> items = EnchantUtils.getInstance().getWeapon(player);
        switch (itemType) {
            case 0:
            switch (sort_type) {
                case 0: items = EnchantUtils.getInstance().getWeapon(player); break;
                case 1: items = EnchantUtils.getInstance().getArmor(player); break;
                case 2: items = EnchantUtils.getInstance().getJewelry(player); break;
            } break;
            case 1:
                ...
                switch (sort_type) {
                    case 0: items = EnchantUtils.getInstance().getScrolls(player); break;
                    case 1: items = EnchantUtils.getInstance().getAtributes(player); break;
                } break;
        }
    ...
    private String getMenuButtons(int activeButton, int item_type) {
        ...
        String[][] itemButtons = { {"Оружие", "Броня", "Бижутерия"},
                {Config.ENCHANT_ALLOW_SCROLLS ? "Заточка" : "unallowed", Config.ENCHANT_ALLOW_ATTRIBUTE ? "Атрибут" : "unallowed"} };
//        int unallowed = 0;
