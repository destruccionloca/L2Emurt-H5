...
public class CastleManorManager {
    ...
    private List<SeedProduction> getNewSeedsList(int castleId) {
        ...
        List<SeedProduction> seeds = new ArrayList<>(allSeeds.size());
        seeds.addAll(allSeeds.stream().map(sd -> new SeedProduction(sd.getId())).collect(Collectors.toList()));
        return seeds;
    }

    private List<CropProcure> getNewCropsList(int castleId) {
        List<SeedData> cropsIds = Manor.getInstance().getCropsForCastle(castleId);
        List<CropProcure> crops = new ArrayList<>(cropsIds.size());
        crops.addAll(cropsIds.stream().map(sd -> new CropProcure(sd.getCrop())).collect(Collectors.toList()));
        return crops;
    }
