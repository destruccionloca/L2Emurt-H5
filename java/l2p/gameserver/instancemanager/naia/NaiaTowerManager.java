...
public final class NaiaTowerManager {
    ...
    public static void removeGroupTimer(Player player) {
        _groupList.keySet().stream().filter(i -> _groupList.get(i).contains(player)).forEach(i -> {
            _groupList.remove(i);
    ...
    private class GroupTowerTimer extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            ...
            if (!_groupList.isEmpty() && !_groupTimer.isEmpty()) {
                _groupTimer.keySet().stream().filter(i -> _groupTimer.get(i) < System.currentTimeMillis()).forEach(i -> {
