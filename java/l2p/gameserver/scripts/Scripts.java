    ...
    private void removeHandlers(Class<?> script) {
        try {
            for (List<ScriptClassAndMethod> entry : dialogAppends.values()) {
                List<ScriptClassAndMethod> toRemove = entry.stream().filter(sc -> sc.className.equals(script.getName())).collect(Collectors.toList());
                toRemove.forEach(entry::remove);
            }
            ...
            toRemove.forEach(onAction::remove);
            ...
            toRemove.forEach(onActionShift::remove);
        } catch (Exception e) {
            _log.error("", e);
        }
    }
