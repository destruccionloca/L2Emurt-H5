...
public class MonsterInstance extends NpcInstance {
    ...
    public void calculateRewards(Creature lastAttacker) {
                ...
                if (quest.getParty() != Quest.PARTY_NONE && players != null) {
                    if (isRaid() || quest.getParty() == Quest.PARTY_ALL) // если цель рейд или квест для всей пати награждаем всех участников
                    {
                        for (Player pl : players) {
                            if (Config.RATE_BARACIEL_KILL_SET_NOOBLE && getNpcId() == 25325) {
                                break;
                            }
        ...
        if (Config.RATE_BARACIEL_KILL_SET_NOOBLE && getNpcId() == 25325){
            List<Player> players = null; // массив с игроками, которые могут быть заинтересованы
            if (killer.getParty() != null) // если пати то собираем всех кто подходит
            {
                ...
                for (Player pl : players) {
                    if (!pl.isNoble()) {
                        Olympiad.addNoble(pl);
                        pl.setNoble(true);
                        pl.updatePledgeClass();
                        pl.updateNobleSkills();
                        pl.sendPacket(new SkillList(pl.getPlayer()));
                        pl.broadcastUserInfo(true);
                    }
                }
            } else {
                if (!killer.isNoble()) {
                    Olympiad.addNoble(killer);
                    killer.setNoble(true);
                    killer.updatePledgeClass();
                    killer.updateNobleSkills();
                    killer.sendPacket(new SkillList(killer));
    ...
    public void rollRewards(Map.Entry<RewardType, RewardList> entry, final Creature lastAttacker, Creature topDamager) {
        ...
        mod *= activePlayer.getRateChance();
    ...
    private void checkUD(Creature attacker, double damage) {
        ...
        if (distance <= MIN_DISTANCE_FOR_CANCEL_UD) {
            if (getEffectList() != null && getEffectList().getEffectsBySkillId(skillId) != null) {
                getEffectList().getEffectsBySkillId(skillId).forEach(Effect::exit);
            }
