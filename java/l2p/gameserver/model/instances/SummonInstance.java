...
public class SummonInstance extends Summon {
    ...
    public int getPDef(Creature target) {
        ...
        return (int) calcStat(Stats.POWER_DEFENCE, _template.basePDef * mod, target, null);
    }

    @Override
    public int getMDef(Creature target, Skill skill) {
        double mod = BaseStats.MEN.calcBonus(this) * getLevelMod();
        return Math.max((int) calcStat(Stats.MAGIC_DEFENCE, _template.baseMDef * mod, target, skill), 1);
    }
    
    @Override
    public int getMaxHp() {
        double mod = BaseStats.CON.calcBonus(this);
        return (int) calcStat(Stats.MAX_HP, _template.baseHpMax * mod, null, null);
    }

    @Override
    public int getMaxMp() {
        double mod = BaseStats.MEN.calcBonus(this);
        return (int) calcStat(Stats.MAX_MP, _template.baseMpMax * mod, null, null);
    }
    ...
    public void onAction(Player player, boolean shift) {
        ...
        if (shift) {
            ...
            String dialog;
            dialog = HtmCache.getInstance().getIfExists("scripts/actions/admin.L2SummonInstance.onActionShift.htm", player);
            ...
            dialog = dialog.replaceFirst("%pAtkSpd%", String.valueOf(getPAtkSpd(true)));
