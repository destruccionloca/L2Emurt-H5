...
public class NpcInstance extends Creature {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        try {
            if (command.equalsIgnoreCase("TerritoryStatus")) {
                ...
                skills.stream().filter(skill -> player.removeSkill(skill.getId(), true, false) != null).forEach(skill -> {
    ...
    public void showQuestWindow(Player player) {
        ...
        if (awaits != null) {
            awaits.stream().filter(x -> !options.contains(x.getQuest())).filter(x -> x.getQuest().getQuestIntId() > 0).forEach(x -> {
    ...
    public boolean isSevenSignsMonster() {
        ...
        return npcId >= 21168 && npcId <= 21207 || getFaction().getName().equalsIgnoreCase("c_dungeon_clan");
    }
    ...
    public void setTargetable(boolean value, boolean cancelTargets) {
        ...
        if ((!value) && (cancelTargets)) {
            World.getAroundPlayers(this).stream().filter(player -> player.getTarget() == this).forEach(player -> {
