...
public final class DoorInstance extends Creature implements GeoCollision {
    ...
    ScheduledFuture<?> _autoActionTask;
    ...
    public void broadcastStatusUpdate() {
        World.getAroundObservers(this).stream().filter(player -> player != null).forEach(player -> {
            player.sendPacket(new StaticObject(this, player));
        });
    }
    ...
    public boolean openMe(Player opener, boolean autoClose) {
        ...
        getListeners().getListeners().stream().filter(l -> l instanceof OnOpenCloseListener).forEach(l -> {
     ...
     */
    boolean setGeoOpen(boolean open) {
        if (_geoOpen == open) {
            return false;
        }
