    ...
    public void spawnSevenSignsNPC() {
        ...
        if (isSealValidationPeriod() || isCompResultsPeriod()) {
            ...
            if (getSealOwner(SEAL_GNOSIS) == getCabalHighestScore() && getSealOwner(SEAL_GNOSIS) != CABAL_NULL) {
                ...
                _oratorSpawns.values().stream().filter(spawnInst -> !AutoSpawnManager.getInstance().getAutoSpawnInstance(spawnInst.getObjectId(), true).isSpawnActive()).forEach(spawnInst -> {
                    AutoSpawnManager.getInstance().setSpawnActive(spawnInst, true);
                });
                _preacherSpawns.values().stream().filter(spawnInst -> !AutoSpawnManager.getInstance().getAutoSpawnInstance(spawnInst.getObjectId(), true).isSpawnActive()).forEach(spawnInst -> {
                    AutoSpawnManager.getInstance().setSpawnActive(spawnInst, true);
                });
            } else {
