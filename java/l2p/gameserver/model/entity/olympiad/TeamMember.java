...
public class TeamMember {
    ...
    public void preparePlayer() {
        ...
        if (Config.OLYMPIAD_BUFF_DELETED) {
            player.getEffectList().getAllEffects().stream().filter(e -> e.getEffectType() != EffectType.Cubic || player.getSkillLevel(e.getSkill().getId()) <= 0).forEach(Effect::exit);
        }
        ...
        player.getSkillReuses().stream().filter(sts -> sts != null).forEach(sts -> {
