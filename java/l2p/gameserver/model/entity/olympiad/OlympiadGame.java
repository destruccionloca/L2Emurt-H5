...
public class OlympiadGame {
    ...
    public void winGame(OlympiadTeam winnerTeam, OlympiadTeam looseTeam) {
        ...
        for (Player player : winnerTeam.getPlayers()) {
            ...
            player.sendChanges();
			player.getInventory().addItem(Config.ALT2_OLY_BATTLE_REWARD_ITEM, Config.ALT2_OLY_BATTLE_REWARD_ITEM_Count);
            player.sendPacket(SystemMessage2.obtainItems(Config.ALT2_OLY_BATTLE_REWARD_ITEM, Config.ALT2_OLY_BATTLE_REWARD_ITEM_Count, 0));
            player.sendChanges();
        }
        ...
        teamsPlayers.stream().filter(player -> player != null).forEach(player -> {
    ...
    public void tie() {
        ...
        for (int i = 0; i < teamMembers1.length; i++) {
            try {
                ...
                member1.getPlayer().getEffectList().getAllEffects().stream().filter(e -> !e.getSkill().isToggle()).forEach(Effect::exit);
                member2.getPlayer().getEffectList().getAllEffects().stream().filter(e -> !e.getSkill().isToggle()).forEach(Effect::exit);
    ...
    public void clearSpectators() {
        _spectators.stream().filter(pc -> pc != null && pc.isInObserverMode()).forEach(pc -> {
    ...
    public void broadcastRelation() {
        _team1.getPlayers().forEach(PlayerUtils::updateAttackableFlags);

        _team2.getPlayers().forEach(PlayerUtils::updateAttackableFlags);
    }
    ...
    public void broadcastPacket(L2GameServerPacket packet, boolean toTeams, boolean toSpectators) {
        ...
        if (toSpectators && !_spectators.isEmpty()) {
            _spectators.stream().filter(spec -> spec != null).forEach(spec -> {
    ...
    public List<Player> getAllPlayers() {
        List<Player> result = _team1.getPlayers().stream().collect(Collectors.toList());
        result.addAll(_team2.getPlayers().stream().collect(Collectors.toList()));
        if (!_spectators.isEmpty()) {
            result.addAll(_spectators.stream().filter(spec -> spec != null).collect(Collectors.toList()));
        }
        return result;
    }
