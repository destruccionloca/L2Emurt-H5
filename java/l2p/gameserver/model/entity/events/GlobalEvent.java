...
public abstract class GlobalEvent extends LoggerObject {
    ...
    private class ListenerListImpl extends ListenerList<GlobalEvent> {
        ...
        public void onStart() {
            getListeners().stream().filter(listener -> OnStartStopListener.class.isInstance(listener)).forEach(listener -> {
    ...
    public void spawnAction(String name, boolean spawn) {
        ...
        objects.stream().filter(object -> object instanceof SpawnableObject).forEach(object -> {
            if (spawn) {
    ...
    public void doorAction(String name, boolean open) {
        ...
        objects.stream().filter(object -> object instanceof DoorObject).forEach(object -> {
            if (open) {
    ...
    public void zoneAction(String name, boolean active) {
        ...
        objects.stream().filter(object -> object instanceof ZoneObject).forEach(object -> {
    ...
    public void initAction(String name) {
        ...
        objects.stream().filter(object -> object instanceof InitableObject).forEach(object -> {
    ...
    public void cloneTo(GlobalEvent e) {
        e._onInitActions.addAll(_onInitActions.stream().collect(Collectors.toList()));

        e._onStartActions.addAll(_onStartActions.stream().collect(Collectors.toList()));

        e._onStopActions.addAll(_onStopActions.stream().collect(Collectors.toList()));
