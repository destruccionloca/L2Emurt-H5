...
public class Coliseum {
    ...
    private void load() {
        _event_cycle = getColiseumMatchNumber();
        Integer localInteger2;
        if (_event_cycle.intValue() == 2147483647) {
            _event_cycle = 1;
        } else {
            Integer localInteger1 = _event_cycle;
            localInteger2 = Coliseum._event_cycle = _event_cycle.intValue() + 1;
        }
    }
            ...
            if (_winner == TeamType.NONE) {
                ...
                if (_party1.getPartyLeader().getTeam() == _winner) {
                    ...
                    if (teleloc == null) {
                        ...
                        setPreviusWinner(null);
                        _party1.getPartyMembers().stream().filter(member -> !member.isDead()).forEach(Player::teleToClosestTown);
                ...
                if (_party2.getPartyLeader().getTeam() == _winner) {
                    ...
                    if (teleloc == null) {
                        ...
                        setPreviusWinner(null);
                        _party2.getPartyMembers().stream().filter(member -> !member.isDead()).forEach(Player::teleToClosestTown);
                    } else if (!isWaitingRoom1Free()) {
                        teleloc = _zone.getRestartPoints().get(1);
