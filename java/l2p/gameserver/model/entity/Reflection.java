...
public class Reflection {
    ...
    public void minuteBeforeCollapse() {
        ...
        lock.lock();
        try {
            _objects.stream().filter(o -> o.isPlayer()).forEach(o -> {
    ...
    public void collapse() {
        ...
        try {
            ...
            _doors.values().forEach(DoorInstance::deleteMe);
            _doors.clear();
    ...
    public List<Player> getPlayers() {
        ...
        lock.lock();
        try {
            result.addAll(_objects.stream().filter(o -> o.isPlayer()).map(o -> (Player) o).collect(Collectors.toList()));
        } finally {
            lock.unlock();
        }
        return result;
    ...
    public List<Creature> getPlayersAndObservers() {
        ...
        this.lock.lock();
        try {
            result.addAll(this._objects.stream().filter(o -> (o.isPlayer()) || (o.isObservePoint())).map(o -> (Creature) o).collect(Collectors.toList()));
        } finally {
            this.lock.unlock();
        }
        return result;
    ...
    public List<Creature> getObservers() {
        ...
        this.lock.lock();
        try {
            result.addAll(this._objects.stream().filter(o -> o.isObservePoint()).map(o -> (Creature) o).collect(Collectors.toList()));
        } finally {
            this.lock.unlock();
        }
        return result;
    ...
    public List<NpcInstance> getNpcs() {
        ...
        lock.lock();
        try {
            result.addAll(_objects.stream().filter(o -> o.isNpc()).map(o -> (NpcInstance) o).collect(Collectors.toList()));
        } finally {
            lock.unlock();
        }
        return result;
    ...
    public List<NpcInstance> getAllByNpcId(int npcId, boolean onlyAlive) {
        ...
        lock.lock();
        try {
            _objects.stream().filter(o -> o.isNpc()).forEach(o -> {
    ...
    public void clearReflection(int timeInMinutes, boolean message) {
        ...
        if (message) {
            getPlayers().stream().filter(pl -> pl != null).forEach(pl -> {
