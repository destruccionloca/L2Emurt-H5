...
public class ShortCutList {
    ...
    public void validate() {
        // Проверка ярлыков
        _shortCuts.values().stream().filter(sc -> sc.getType() == ShortCut.TYPE_ITEM).filter(sc -> player.getInventory().getItemByObjectId(sc.getId()) == null).forEach(sc -> {
    ...
    public void deleteShortCutByObjectId(int objectId) {
        _shortCuts.values().stream().filter(shortcut -> shortcut != null && shortcut.getType() == ShortCut.TYPE_ITEM && shortcut.getId() == objectId).forEach(shortcut -> {
    ...
    public void deleteShortCutBySkillId(int skillId) {
        _shortCuts.values().stream().filter(shortcut -> shortcut != null && shortcut.getType() == ShortCut.TYPE_SKILL && shortcut.getId() == skillId).forEach(shortcut -> {
            deleteShortCut(shortcut.getSlot(), shortcut.getPage());
        });
    }
