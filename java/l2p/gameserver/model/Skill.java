        ...
        if ((player != null) && (getId() != 1505)) {
            ...
            if (pcTarget != null) {
                ...
                if (isOffensive()) {
					...
					if (target.isSummon() && (target == activeChar.getPet()) && isPvpSkill())
                        {
                            return SystemMsg.INVALID_TARGET;
                        }
        ...
        switch (_targetType) {
            ...
            case TARGET_CORPSE:
                if (target == null || !target.isDead()) {
                    activeChar.sendPacket(new SystemMessage(SystemMsg.INVALID_TARGET));
                    return null;
                }
                return target.isNpc() || target.isSummon() ? target : null;
        ...
        switch (_targetType) {
            ...
            case TARGET_COMMCHANNEL: {
                if (activeChar.getPlayer() != null) {
                    if (activeChar.getPlayer().isInParty()) {
                        ...
                        targets.addAll(activeChar.getPlayer().getParty().getPartyMembers().stream().filter(p -> !p.isDead() && p.isInRange(activeChar, _skillRadius == 0 ? 600 : _skillRadius)).collect(Collectors.toList()));
                        addTargetAndPetToList(targets, activeChar.getPlayer(), activeChar.getPlayer());
                        break;
                    }
            ...
            case TARGET_ALLY: {
                if (activeChar.isMonster() || activeChar.isSiegeGuard()) {
                    targets.add(activeChar);
                    /*&& ((L2MonsterInstance) c).getFactionId().equals(mob.getFactionId())*/
                    targets.addAll(World.getAroundCharacters(activeChar, _skillRadius, 600).stream().filter(c -> !c.isDead() && (c.isMonster() || c.isSiegeGuard())).collect(Collectors.toList()));
                    break;
                }
                Player player = activeChar.getPlayer();
                if (player == null) {
            ...
            case TARGET_FRIEND: {
                if (activeChar.isMonster() || activeChar.isSiegeGuard()) {
                    targets.add(activeChar);
                    targets.addAll(World.getAroundCharacters(activeChar, _skillRadius, 900).stream().filter(c -> !c.isDead() && (c.isMonster() || c.isSiegeGuard())).collect(Collectors.toList()));
                    break;
                }
                Player player = activeChar.getPlayer();
