...
public class Manor {
    ...
    public List<Integer> getAllCrops() {
        ...
        _seeds.values().stream().filter(seed -> !crops.contains(seed.getCrop()) && seed.getCrop() != 0 && !crops.contains(seed.getCrop())).forEach(seed -> {
    ...
    public List<SeedData> getCropsForCastle(int castleId) {
        List<SeedData> crops = new ArrayList<>(20);
        crops.addAll(_seeds.values().stream().filter(seed -> seed.getManorId() == castleId).collect(Collectors.toList()));
        return crops;
    }
    ...
    public List<SeedData> getSeedsForCastle(int castleId) {
        List<SeedData> seeds = new ArrayList<>(20);
        seeds.addAll(_seeds.values().stream().filter(seed -> seed.getManorId() == castleId).collect(Collectors.toList()));
        return seeds;
    }
