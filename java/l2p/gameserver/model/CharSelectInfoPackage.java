...
public class CharSelectInfoPackage {
    ...
    public CharSelectInfoPackage(int objectId, String name) {
        ...
        //FIXME [G1ta0] временный фикс отображения одетых вещей при входе на персонажа в NO CARRIER
        items.stream().filter(item -> item.getEquipSlot() < Inventory.PAPERDOLL_MAX).forEach(item -> {
    ...
    public int getPaperdollItemId(int slot) {
        ...
        if (item != null) {
            ...
            if ((slot == 11 || slot == 6 || slot == 12 || slot == 9) && (paperdollItem = this._paperdoll[10]) != null && (paperdollItem.getCustomFlags() & 2048) == 2048) {
                return 0;
            }
