...
public abstract class Playable extends Creature {
    ...
    public void doCast(final Skill skill, final Creature target, boolean forceUse) {
        ...
        if ((skill.isNotTargetAoE()) && (skill.isOffensive()) && (isInPeaceZone())) {
            player.sendPacket(Msg.A_MALICIOUS_SKILL_CANNOT_BE_USED_IN_A_PEACE_ZONE);
            return;
        }

        if ((skill.isPvpSkill()) && (target.isNpc()) && (!target.isMonster()) && (!target.isAutoAttackable(this))) {
            player.sendPacket(Msg.INVALID_TARGET);
            return;
        }
    ...
    public void reduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp, boolean canReflect, boolean transferDamage, boolean isDot, boolean sendMessage) {
        ...
        if (attacker != this && attacker.isPlayable()) {
            ...
            //дамаг по своим на ивентах не проходит
            if (player.isInPvPEventNoLh() && pcAttacker.isInPvPEvent() && player.getTeam() == pcAttacker.getTeam()) {
                if (sendMessage) {
                    pcAttacker.sendPacket(Msg.INVALID_TARGET);
                }
                return;
    ...
    public int getPAtkSpd(boolean applyLimit) {
        int result = Math.max((int) calcStat(Stats.POWER_ATTACK_SPEED, calcStat(Stats.ATK_BASE, this._template.basePAtkSpd, null, null), null, null), 1);
    ...
    public boolean isCtrlAttackable(Creature attacker, boolean force, boolean witchCtrl) {
        ...
        if (pcAttacker != null && pcAttacker != player) {
            ...
            //TODO [VISTALL] что за?
            if (pcAttacker.getTeam() != TeamType.NONE && pcAttacker.isChecksForTeam() && player.getTeam() == TeamType.NONE) // Запрет на атаку/баф участником эвента незарегистрированного игрока
            {
                return false;
            }
            if (player.getTeam() != TeamType.NONE && player.isChecksForTeam() && pcAttacker.getTeam() == TeamType.NONE) // Запрет на атаку/баф участника эвента незарегистрированным игроком
            {
                return false;
            }
            if (player.getTeam() != TeamType.NONE && player.isChecksForTeam() && pcAttacker.getTeam() != TeamType.NONE && pcAttacker.isChecksForTeam() && player.getTeam() == pcAttacker.getTeam()) // Свою команду атаковать нельзя
            {
                return false;
            } 
            for (GlobalEvent e : attacker.getEvents()) {
    ...
    public void unparalizeMe(Creature effector) {
        if (effector.getEffectList().getEffectsBySkillId(Skill.SKILL_RAID_CURSE) != null) {
            effector.getEffectList().stopEffect(SkillTable.getInstance().getInfo(Skill.SKILL_RAID_CURSE, 1));
        }
    }
