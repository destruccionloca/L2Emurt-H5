...
public class Clan implements Iterable<UnitMember> {
    ...
    public static long EXPELLED_MEMBER_PENALTY = Config.EXPELLED_MEMBER_PENALTY * 1 * 60 * 60 * 1000L;
    public static long LEAVED_ALLY_PENALTY = Config.LEAVED_ALLY_PENALTY * 1 * 60 * 60 * 1000L;
    public static long DISSOLVED_ALLY_PENALTY = Config.DISSOLVED_ALLY_PENALTY * 1 * 60 * 60 * 1000L;
    ...
    public void enableSkills(Player player) {
        ...
        _skills.values().stream().filter(skill -> skill.getMinPledgeClass() <= player.getPledgeClass()).forEach(player::removeUnActiveSkill);
    ...
    public List<L2GameServerPacket> listAll() {
        ...
        p.addAll(getAllSubUnits().stream().map(unit -> new PledgeShowMemberListAll(this, unit)).collect(Collectors.toList()));
    ...
    public Iterator<UnitMember> iterator() {
        ...
        iterators.addAll(_subUnits.values().stream().map(subUnit -> subUnit.getUnitMembers().iterator()).collect(Collectors.toList()));
