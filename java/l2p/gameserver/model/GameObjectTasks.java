...
public class GameObjectTasks {
    ...
    public static class AcademPlayerTask extends RunnableImpl {

        private final HardReference<Player> _playerRef;

        public AcademPlayerTask(Player player) {
            _playerRef = player.getRef();
        }
            ...
            if (player != null) {
                ...
                if (clan != null) {
                    if (clan.getLeader().isOnline()) {
                        AcademList.deleteAcademInDb(clan, player.getObjectId(), false, true);
                    }
                    ...
                    // disable clan tab
                    player.sendPacket(SystemMsg.YOU_HAVE_RECENTLY_BEEN_DISMISSED_FROM_A_CLAN, new PledgeShowMemberListDeleteAll());
                }
            }
        }
    }
