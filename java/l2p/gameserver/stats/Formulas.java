...
public class Formulas {
    ...
    public static void calcStunBreak(Creature target, boolean crit) {
        if (target.isStunned() && Rnd.chance(crit ? 75 : 10)) {
            target.getEffectList().getAllEffects().stream().filter(e -> e.getEffectType() == EffectType.Stun).forEach(e -> {
                e.exit();
                if (target.isPlayer()) {
        ...
        {
            target.getEffectList().getAllEffects().stream().filter(e -> e.getSkill().getId() == 522).forEach(e -> {
                e.exit();
                if (target.isPlayer()) {
                    target.sendPacket(new SystemMessage2(SystemMsg.THE_EFFECT_OF_S1_HAS_BEEN_REMOVED).addSkillName(e.getSkill()));
                }
