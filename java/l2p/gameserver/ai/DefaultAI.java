...
public class DefaultAI extends CharacterAI {
        ...
        if (player != null) {
            ...
            if ((SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod()) && actor.isSevenSignsMonster() && (!Config.ALLOW_SS_TP_NO_REG)) {
                ...
                if (pcabal != wcabal && wcabal != SevenSigns.CABAL_NULL) {
                    player.sendMessage(player.isLangRus()? "Вы телепортированы до ближайшего города, потому что вы не зарегистрированы за победившую печать.": "You have been teleported to the nearest town because you not signed for winning cabal.");
                    player.teleToClosestTown();
                    return;
                }
            }
            if(pcabal == SevenSigns.CABAL_NULL && actor.isSevenSignsMonster() && Config.ALT_SEVEN_SIGNS_NO_REG){
                player.sendMessage(player.isLangRus()? "Вы телепортированы до ближайшего города, потому что вы не зарегистрированы за победившую печать.": "You have been teleported to the nearest town because you not signed for winning cabal.");
                player.teleToClosestTown();
                return;
            }
        ...
        if (System.currentTimeMillis() - _lastFactionNotifyTime > _minFactionNotifyInterval) {
            ...
            if (actor.isMinion()) {
                ...
                if (master != null) {
                    ...
                    if (minionList != null) {
                        minionList.getAliveMinions().stream().filter(minion -> minion != actor).forEach(minion -> {
    ...
    protected List<NpcInstance> activeFactionTargets() {
        ...
        return World.getAroundNpc(actor).stream().filter(npc -> !npc.isDead()).filter(npc -> npc.isInFaction(actor)).filter(npc -> npc.isInRangeZ(actor, npc.getFaction().getRange())).filter(npc -> GeoEngine.canSeeTarget(npc, actor, false)).collect(Collectors.toCollection(ArrayList::new));
    }
