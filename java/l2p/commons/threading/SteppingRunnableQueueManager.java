...
public abstract class SteppingRunnableQueueManager implements Runnable {
    ...
    public void run()
    {
        ...
        try
        {
            ...
            queue.stream().filter(sr -> !sr.isDone()).forEach(SteppingScheduledFuture::run);
        }
        finally
        {
            isRunning.set(false);
    ...
    public void purge()
    {
        List<SteppingScheduledFuture<?>> purge = queue.stream().filter(SteppingScheduledFuture::isDone).collect(Collectors.toList());
